...
<title>学生提交课题内容检验</title>
</head>
<body>
	<%
		request.setCharacterEncoding("UTF-8");
		String title_id = (String) request.getParameter("no");
		String textarea = (String) request.getParameter("textare");
		...
		for (Title tl : list) {
			if (tl.getTitle_id().equals(title_id)) {
				...
				try {
					...
					String sql;
					sql = "UPDATE title SET stu_title='" + textarea + "'  WHERE title_id='" + title_id + "'";
					stmt.executeUpdate(sql);
					out.print("<script type='text/javascript' language='javaScript'> alert('学生提交课题内容成功');</script>");
				} finally {
					// 完成后关闭
					manager.disconnect(conn);
				}
				response.setHeader("refresh", "0;url=stutitle.jsp");
			}
		}
	%>
</body>
